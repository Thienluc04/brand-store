...
export function Header() {
  ...
  const hanldeSubmitSearch = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (inputRef.current) {
      ...
      dispatch(productActions.setSearchQuery(input.value));
    }
  };
  ...
  // const lengthCart = useAppSelector(selectCartLength);

  const mycart = localStorage.getItem("my_cart");

  useEffect(() => {
    if (mycart) {
      const cart = Array.from(JSON.parse(String(mycart)));
          ...
          {currentUser ? (
            <>
              <p className="hidden xl:block">
                Hello <span className="text-primary">{currentUser.displayName}</span>
              </p>
